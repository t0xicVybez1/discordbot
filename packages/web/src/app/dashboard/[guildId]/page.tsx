          ...
          <div className="space-y-3">
            {[
              ...
              <div key={feature.key} className="flex items-center justify-between">
                ...
                <span className={`badge ${(guild as unknown as Record<string, unknown>)?.settings && ((guild as unknown as Record<string, Record<string, unknown>>).settings)?.[feature.key] ? 'badge-success' : 'badge-danger'}`}>
                  {(guild as unknown as Record<string, unknown>)?.settings && ((guild as unknown as Record<string, Record<string, unknown>>).settings)?.[feature.key] ? 'Enabled' : 'Disabled'}
                </span>
              </div>
            ))}
          </div>
